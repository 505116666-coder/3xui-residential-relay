...
已安装新版后，运行 `relay update` 或选择菜单 **9** 更新管理脚本。下载固定提交并校验 SHA-256，检查新版能否启动后替换；旧版保存在 `/root/3xui-dual/manager.previous.py`。更新不升级面板、不修改节点。

菜单等待输入时不占操作锁；改名仅提交节点备注，不主动重启服务；检查结果分别显示 TCP 和 UDP 状态。

首次从旧版更新：重新运行上方下载命令并在末尾加 `--results`，即可更新管理器并安装短命令；保留现有节点。旧安装文件名仍可使用。
